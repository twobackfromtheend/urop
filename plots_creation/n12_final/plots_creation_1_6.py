...
protocols = {
    # (1, 'std', 0.8, 0.05): [6.34454727e+08, 7.64928934e+08, 9.34283468e+08, 1.02156056e+09, 1.89721547e+09, 1.44314419e+09],

     # (2, 'std', 0.8, 0.00): [4.57974088e+08, 5.35955927e+08, 1.78762850e+09, 2.45732764e+09, 3.16083220e+09, 2.40855530e+09],

    # (2, 'std', 0.8, 0.05): [1.93468855e+09, 1.90027146e+09, 1.33490242e+09, 2.94394265e+09, 3.10754913e+09, 2.76994896e+09],
    # (2, 'alt', 0.8, 0.05): [5.87287276e+07, 1.79912504e+08, 1.80608135e+08, 1.54127381e+08, 2.63066167e+08, 1.50634293e+08],

    # (1, 'std', 0.8, 0.03): [1.07472148e+09, 1.25357060e+09, 1.31251964e+08, 1.25042057e+09, 1.56105307e+09, 1.13539811e+09],
    # (1, 'alt', 0.8, 0.03): [6126057.27592245, 7928155.24079046, 17567652.13275377, 16496542.07221888, 24182412.50202318, 12663133.37825134],
    # (2, 'std', 0.8, 0.03): [5.23951266e+08, 2.21496887e+09, 1.56304224e+09, 2.74362023e+09, 2.19698970e+09, 2.26371010e+09],
    (3, 'std', 0.8, 0.03): [4.82237827e+08, 1.41301379e+09, 2.35345829e+09, 2.53032774e+09, 3.50787444e+09, 2.43481381e+09],
    # (3, 'alt', 0.8, 0.03): [1.28637664e+08, 2.84159011e+08, 1.85497057e+08, 4.40250872e+08, 3.04509848e+08, 2.64256094e+08,],
...
def get_geometry_ghz_state(D, ghz):
    if D == 1:
        shape = (12,)
        if ghz == "std":
...
def _plot_protocol_and_fidelity(ax1: Axes, ax2: Axes, e_qs: EvolvingQubitSystem, tukey_alpha, input_, noise):
    omegas_ = []
    deltas_ = []
    foms_ = []
    rho_00s_ = []
    rho_11s_ = []
    off_diags_ = []
    pg = CustomInterpolationPG(e_qs.t_list, kind="cubic", tukey_alpha=tukey_alpha, noise=noise)
    np.random.seed(278784234)
    for i in trange(20):
        ...
        off_diags = []
        foms_skip = 10
        for _instantaneous_state in e_qs.solved_states[::foms_skip]:
        ...
        tqdm.write(f"fom: {foms[-1]:.3f} \t{rho_00s[-1]:.3f} \t{rho_11s[-1]:.3f} \t{off_diags[-1]:.3f}")
...
def plot_BO_geometries_and_GHZ():
    ...
    for (D, ghz, tukey_alpha, noise), input_ in protocols.items():
        geometry, ghz_state = get_geometry_ghz_state(D, ghz)
        t = 1e-6 if ghz == "alt" else 1e-7
        e_qs = EvolvingQubitSystem(
            N=N, V=C6, geometry=geometry,
            Omega=np.linspace(0, t, 3000),
            Delta=np.linspace(0, t, 3000),
        ...
        save_current_fig(f"_noise_optimised_{noise}_new_protocol_and_fidelity_{D}d_{ghz}")
