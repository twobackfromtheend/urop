...
def get_ground_states(N: int) -> List[q.qarray]:
    return [q.ket([0, 1]) for _ in range(N)]


def get_excited_states(N: int) -> List[q.qarray]:
    return [q.ket([1, 0]) for _ in range(N)]


def get_states(N: int) -> List[List[q.qarray]]:
    """
    ...
    """
    states = []
    g = get_ground_states(1)[0]
    e = get_excited_states(1)[0]
