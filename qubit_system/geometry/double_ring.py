...
class DoubleRing(BaseGeometry):
    def __init__(self, N: int, spacing: float = 1):
        ...
        unit_ring_coordinates = []
        for i in range(N_per_ring):
            theta = angular_separation_between_qubits * i
            unit_ring_coordinates.append((np.cos(theta), np.sin(theta)))
        unit_ring_coordinates = np.array(unit_ring_coordinates)

        distance_between_qubits = np.sum((unit_ring_coordinates[0] - unit_ring_coordinates[1]) ** 2) ** 0.5
        ...
        for i in range(rings):
            ...
            coordinates.append(unit_ring_coordinates * _scaling_factor)
...
if __name__ == '__main__':
    ...
    geometry = DoubleRing(8, spacing=1)
    geometry.plot(show=True, savefig_name="geometry_double_ring_8")
